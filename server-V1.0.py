...
# Function to check if all clients have sent data and run OS commands if true
def check_and_run_commands():
    global client_flags, client_hashes

    while True:
        with connections_lock:
            if len(client_flags) >= expected_clients:
                # Change directory
...
# Start the thread to check and run OS commands
